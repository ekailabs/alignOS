...
Deno.test("owner envelope gates owner-only routes", async () => {
  ...
  const body = JSON.stringify({
    pairs: [{ prompt: "hello", output: "world" }],
  });
  const ownerPriv = ed25519.utils.randomPrivateKey();
  const otherPriv = ed25519.utils.randomPrivateKey();

  if (
    owner.verifyOwner("POST", path, body, headersFor(ownerPriv, path, body))
  ) {
  ...
  if (
    owner.verifyOwner("POST", path, body, headersFor(otherPriv, path, body))
  ) {
  ...
  if (
    owner.verifyOwner(
      ...
      path,
      "{}",
      headersFor(ownerPriv, path, body, "nonce-wrong-body"),
    )
  ) {
  ...
  if (
    !owner.verifyOwner(
      ...
      path,
      body,
      headersFor(ownerPriv, path, body, "nonce-ok"),
    )
  ) {
...
Deno.test("claim can rebind stale URL-only owner state", async () => {
  const dir = await Deno.makeTempDir();
  Deno.env.set("ALIGN_OWNER_STATE", `${dir}/owner.json`);
  const owner = await import(`./owner.ts?test=${crypto.randomUUID()}`);
  ...
  if (
    !owner.verifyOwner(
      ...
      path,
      body,
      headersFor(firstPriv, path, body, "nonce-first"),
    )
  ) {
  ...
  if (!second.ok || !second.rebound) {
    throw new Error("second claim should rebind stale owner state");
  }
  if (
    owner.verifyOwner(
      "POST",
      path,
      body,
      headersFor(firstPriv, path, body, "nonce-still-first"),
    )
  ) {
    throw new Error("first key should no longer be accepted after rebind");
  }
  if (
    !owner.verifyOwner(
      "POST",
      path,
      body,
      headersFor(secondPriv, path, body, "nonce-second"),
    )
  ) {
    throw new Error("second key should be accepted after rebind");
  }
});
